...
(function (w) {
    ...
    console.log("gl-element.js constructor");
    ...
    p.init = function () {
        console.log("gl-element.js init");
        this.notifyGlParent();
        this.updateTexture();
        this.initObservers();
    ...
    p.dispatchReady = function () {
        //console.trace("POTENTIALLY Dispatching ready event : ", (this.glChilds.length - this.glChildsReady === 0), (this.haveSprite() != undefined), this.isReady(), this, this.glParent);
        if (this.isReady()) {
            var event = new Event(HTMLGL.READY_EVENT);
            this.dispatchEvent(event);
            //console.trace("Dispatching ready event : ", this, this.glParent);
    ...
    p.isReady = function () {
        return (this.glChilds.length - this.glChildsReady === 0);// && (this.haveSprite() != undefined); // changed by Robin, never had a sprite... 
    }
    ...
    p.initObservers = function () {
        ...
        self.updatingTexture = false;// added by robin
        self.textureUpdateRequested = false;// added by robin
        ...
        this.observer = this.observer || new MutationObserver(function (mutations) {
            ...
            mutations.forEach(function (mutation) {
                ...
                var spriteUpdate = false;
                if( computedStyle.transform != "matrix(1, 0, 0, 1, 0, 0)")
                    spriteUpdate = true;

                if (mutation.attributeName === 'style' && spriteUpdate) {
                    
                    rS('html-gl-sprite').start();
                    self.transformObject = self.getTransformObjectFromString(self.style[self.transformProperty]);
                    self.updateSpriteTransform();
                    console.log("Updating sprite! took " + rS('html-gl-sprite').end() + "ms " );
                } else {

                    if( self.updatingTexture )
                    {
                        self.textureUpdateRequested = true;
                    }
                    else
                    {
                        self.updatingTexture = true;
                        
                        rS('html-gl-tex').start();
                        console.log("Updating texture! ");
                        var onDone = function () { 
                            self.updatingTexture = false; 
                            console.log("Texture updated! took " + rS('html-gl-tex').end() + "ms");  

                            // little bit dirty, but easiest without big refactor right now // added by robin
                            if( self.textureUpdateRequested )
                            {
                                self.textureUpdateRequested = false;
                                self.updatingTexture = true;
                                rS('html-gl-tex').start();
                                self.updateTexture().then(onDone);
                            }
                        };
                        self.updateTexture().then(onDone);
                    }
    ...
    p.domHasBeenHidden = false; // added by Robin
    p.hideDOM = function () {
        if( !p.domHasBeenHidden ) // added by Robin. Kept triggering useless MutationRecords otherwise after each texture update 
        {
            this.style.opacity = 0;
            p.domHasBeenHidden = true; // added by Robin
        }
    }
